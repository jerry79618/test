...
**Header/信封選擇規則**（LBSystem 確定後）：
1. 查 `.cucb/config.md`「系統協定設定」表該 LBSystem 列，依 `Header Helper` / `Response 類型` 欄決定請求信封的實作類別
2. **表不存在**（舊 config）→ 一律視為原生 CBK（`CBKHeaderHelper` / `CBKResponse`），維持既有行為
3. 表存在但該列 Header Helper 標 `⚠️ 待補`，或 HeaderType 在測試框架中無對應 Helper/Client → 依「協定不相容時的骨架 Helper」規範（stepCode.instructions.md）產出骨架並標注 `[CONNECTION GAP]`，同時列入 `clarifications_needed` 回報，**禁止套用 CBK 信封硬打非 CBK 系統**
